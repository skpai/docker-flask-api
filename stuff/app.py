...
# hostname = os.getenv("summarizer")
summarizer_address = os.environ.get("summarizer_address")
...
url = f"http://{summarizer_address}/model/predict"
...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://wym_admin:admin@db:5432/postgres"
...
# class User:
#     def __init__(self, fname, email="", lname="", message=""):
#         self.fname = fname
#         self.lname = lname
#         self.email = email
#         self.message = message
...
#     def get_df(self, df):
#         df.append((self.fname, self.lname), ignore=True)
#         return df
...
#     host="localhost", port=5432, database="postgres", user="wym_admin", password="admin"
# )
...
def contactme():
    ...
    # df.append([user.fname, user.lname])
    ...
    # json.dumps(messagedict)
    response = requests.post(url, json=messagedict)
    print(response.content)
    db.session.commit()
    return render_template(
        "success.html", data=response.content
